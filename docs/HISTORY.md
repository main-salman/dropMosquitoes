...
- **[PROCUREMENT]** Part selected: Seloky "Dual High-Power MOSFET Trigger Switch Drive Module" (D4184-class dual-MOS, DC 5–36V, 15A/400W, **digital trigger 3.3–20V** — confirms 3.3V GPIO compatibility, explicitly rated for solenoid valves). ~$15–19 CAD on Amazon.ca. Diagram generalized from "D4184" to "dual-MOSFET module" to match; trigger pin may be labeled PWM/TRIG = SIG. Module does NOT include a load flyback → external 1N5408 still required. 12V power-in from existing bus (not the 5V servo rail).
